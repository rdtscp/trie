...
public:
  /* Takes in a string representing the remaining portion of the word, and
   * creates a structure accordingly. */
  ...
  /* Used recursively to determine if a prefix of a string exists within the
   * parent Trie. */
  bool has_prefix(const std::string &prefix);

  /* Get all possible extensions of a prefix. If an invalid prefix is supplied,
